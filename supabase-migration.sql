...
-- 11. PREMIUM SUBMISSIONS (manual transfer + admin verification)
...
CREATE POLICY "Users can view own submissions"
  ON premium_submissions FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own submissions"
  ON premium_submissions FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Admins can view all submissions"
  ON premium_submissions FOR SELECT USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
  );

CREATE POLICY "Admins can manage submissions"
  ON premium_submissions FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
  );
...
-- 14. Tambah kolom expires_at di profiles
